import greenfoot.*;

public class Button_B extends Actor {
    private GreenfootImage Original = new GreenfootImage("Button_B.png");
    private GreenfootImage Correct = new GreenfootImage("Button_B Correct.png");
    private World currentWorld;

    public Button_B(World world) {
        this.currentWorld = world;
        setImage(Original);
    }

    public void act() {
        if (Greenfoot.mouseClicked(this)) {
            if (currentWorld instanceof Question1World && !((Question1World) currentWorld).isButtonClicked()) {
                setImage(Correct);
                ((Question1World) currentWorld).setButtonClicked(true);
                ((Question1World) currentWorld).showNextButton();
            } else if (currentWorld instanceof Question2World && !((Question2World) currentWorld).isButtonClicked()) {
                setImage(Correct);
                ((Question2World) currentWorld).setButtonClicked(true);
                ((Question2World) currentWorld).showNextButton();
            } else if (currentWorld instanceof Question3World && !((Question3World) currentWorld).isButtonClicked()) {
                setImage(Correct);
                ((Question3World) currentWorld).setButtonClicked(true);
                ((Question3World) currentWorld).showNextButton();
            }
        }
    }
}
